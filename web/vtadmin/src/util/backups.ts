 ...
 */
export const BACKUP_STATUSES = invert(pb.Backup.Status);
 ...
 */
export const formatStatus = (status: pb.Backup.Status | null | undefined) => {
    if (typeof status !== 'number' || !(status in BACKUP_STATUSES)) {
        return BACKUP_STATUSES[pb.Backup.Status.UNKNOWN];
    }
