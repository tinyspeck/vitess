...
func (txe *TxExecutor) Prepare(transactionID int64, dtid string) error {
	...
	localConn, err := txe.te.txPool.LocalBegin(txe.ctx, false, false)
	if err != nil {
		return err
	}
...
func (txe *TxExecutor) markFailed(ctx context.Context, dtid string) {
	...
	conn, err := txe.te.txPool.LocalBegin(ctx, false, false)
	if err != nil {
...
func (txe *TxExecutor) RollbackPrepared(dtid string, originalID int64) error {
	...
	conn, err := txe.te.txPool.LocalBegin(txe.ctx, false, false)
	if err != nil {
		goto returnConn
	}
